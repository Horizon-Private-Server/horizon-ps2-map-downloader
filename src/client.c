  ...
  switch (hp->h_addrtype) {
    case AF_INET:
      ...
      for(int i = 0; p1[i]!=NULL; i+=1) {
        //memcpy(&ipaddr, p1[i], sizeof(u32));
        char *s_addr_c = (char*)&p1[i]->s_addr;
        ipaddr_c[0] = s_addr_c[3];
        ipaddr_c[1] = s_addr_c[2];
        ipaddr_c[2] = s_addr_c[1];
        ipaddr_c[3] = s_addr_c[0];
      }
      break;
  }
...
int client_connect(const char* hostname)
{
  ...
  u32 ipaddr = resolveHostname(hostname);

  mysocket = lwip_socket(AF_INET, SOCK_STREAM, 0);

  memset(&dest, 0, sizeof(dest));                /* zero the struct */
  dest.sin_family = AF_INET;
  dest.sin_addr.s_addr = htonl(ipaddr); /* set destination IP number - localhost, 127.0.0.1*/ 
  dest.sin_port = htons(80);                /* set destination port number */

  if (lwip_connect(mysocket, (struct sockaddr *)&dest, sizeof(struct sockaddr_in)) < 0)
    return -1;
...
int client_get(int socket, const char* hostname, const char* path, char* output, int output_size)
{
  ...
  int len = lwip_send(socket, buffer, strlen(buffer), 0);
  DPRINTF("sent %d\n", len);
  len = lwip_recv(socket, output, output_size, 0);
